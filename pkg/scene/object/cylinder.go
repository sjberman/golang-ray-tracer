...
func (cyl *Cylinder) DeepCopy() Object {
	newObj := NewCylinder()
	newObj.Closed = cyl.Closed
	newObj.Minimum = cyl.Minimum
	newObj.Maximum = cyl.Maximum

	newMaterial := cyl.Material
	newObj.SetMaterial(&newMaterial)
	newTransform := cyl.transform
